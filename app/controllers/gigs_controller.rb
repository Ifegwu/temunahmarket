class GigsController < ApplicationController
  ...
  before_action :set_category, only: [:edit, :show, :new]
  ...
  def update
    ...
    if @gig.update(gig_params)
      ...
      return redirect_to request.referrer, flash: {error: @gig.error.full_messages}
    end
  ...
  def set_step
    @step = params[:step].to_i > 0 ? @step = params[:step].to_i : 1
    if @step > 5
      @step = 5
    end
  end
